	...
	public class MongoCorruptionPreventionService
	{
		...
		private async void PerformHealthCheck(object state)
		{
			try
			{
				...
				_logger.LogInformation("🔍 MongoDB corruption prevention check #{_checksPerformed} at {timestamp}", _checksPerformed, timestamp.ToString("HH:mm:ss"));
		...
		private async Task HandleCorruptionIssuesAsync(List<CorruptionIssue> issues)
		{
			...
			_logger.LogWarning(
				"🚨 CORRUPTION DETECTED: {total} issues ({high} high, {medium} medium)",
				issues.Count,
			...
			{
				_logger.LogWarning("🔴 [{severity}] {collection}: {description}", issue.Severity, issue.Collection, issue.Description);
			}
