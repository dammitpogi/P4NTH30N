...
public class FixData
{
	public static void Main(string[] args)
	{
		...
		try
		{
			...
			double maxGrandFound = 0;
			double maxThresholdFound = 0;
			int updatedCount = 0;
			int dpdCleanedCount = 0;

			foreach (var credential in credentials)
			{
				if (credential.Thresholds.Grand > maxThresholdFound) maxThresholdFound = credential.Thresholds.Grand;
				
				if (credential.DPD?.Data != null)
				{
					foreach(var d in credential.DPD.Data)
					{
						if (d.Grand > maxGrandFound) maxGrandFound = d.Grand;
					}
				}
			...
			Console.WriteLine($"\nCleanup complete!");
			Console.WriteLine($"Max Grand in DPD: {maxGrandFound}");
			Console.WriteLine($"Max Threshold Grand: {maxThresholdFound}");
