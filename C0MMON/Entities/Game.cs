...
public class Game(string house, string game) {
    ...
    public static Game Get(string house, string game) {
        ...
        FilterDefinition<Game> filter = Builders<Game>.Filter.Eq(x => x.House, house) & Builders<Game>.Filter.Eq(x => x.Name, game);
        Game? dto = null;
        while (true) {
    ...
    public static Game? GetNext(string Name) {
        FilterDefinitionBuilder<Game> builder = Builders<Game>.Filter;
        FilterDefinition<Game> filter = builder.Eq("Name", Name) & builder.Eq("Unlocked", true) & builder.Eq("Enabled", true) & builder.Or([builder.Eq("Updated", false), builder.Eq("Updated", BsonNull.Value)]);
        List<Game> games = new Database().IO.GetCollection<Game>("G4ME").Find(filter).SortBy(g => g.LastUpdated).Limit(1).ToList();
        return games.Count > 0 ? games[0] : null;
    }
    ...
    public static class GetBy {
        public static Game House(string house) {
            return new Database().IO.GetCollection<Game>("G4ME").Find(Builders<Game>.Filter.Eq("House", house)).ToList()[0];
        }
    }
