...
public class Signal(float priority, Credential credential) {
    ...
    public static List<Signal> GetAll() {
        return new Database().IO.GetCollection<Signal>("SIGN4L").Find(Builders<Signal>.Filter.Empty).ToList();
    }
    ...
    public void Save() {
        ...
        FilterDefinition<Signal> filter = builder.Eq("House", House) & builder.Eq("Game", Game) & builder.Eq("Username", Username);
        List<Signal> dto = new Database().IO.GetCollection<Signal>("SIGN4L").Find(filter).ToList();
        if (dto.Count.Equals(0)) new Database().IO.GetCollection<Signal>("SIGN4L").InsertOne(this);
        else {
            _id = dto[0]._id;
            new Database().IO.GetCollection<Signal>("SIGN4L").ReplaceOne(filter, this);
        }
