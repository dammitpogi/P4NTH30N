...
public static partial class Games {
    public static void Quintuple5X(ChromeDriver driver, Game game, Signal signal) {
        ...
        float balance = 0; int iterations = 0; bool slotsLoaded = false;
        ...
        while (slotsLoaded == false) {
            ...
            if (Screen.WaitForColor(new Point(957, 602), Color.FromArgb(255, 255, 255, 255), 30) == false) {
        ...
        Mouse.Click(955, 290);
        bool jackpotPopped = false; int FailedSpinChecks = 0, remainingIterations = 30;
        double balancePrior = 0, grandPrior = 0, majorPrior = 0, minorPrior = 0, miniPrior = 0;
        ...
        while (true) {
            ...
            game = Game.Get(signal.House, signal.Game);
            balance = Convert.ToSingle(driver.ExecuteScript("return window.parent.Balance")) / 100;
            game.Jackpots.Grand = Convert.ToSingle(driver.ExecuteScript("return window.parent.Grand")) / 100F;
            game.Jackpots.Major = Convert.ToSingle(driver.ExecuteScript("return window.parent.Major")) / 100F;
            game.Jackpots.Minor = Convert.ToSingle(driver.ExecuteScript("return window.parent.Minor")) / 100F;
            game.Jackpots.Mini = Convert.ToSingle(driver.ExecuteScript("return window.parent.Mini")) / 100F;
            ...
            if (jackpotPopped) {
                if (remainingIterations.Equals(30)) {
                    signal.Delete();
                } else remainingIterations--;
                if (remainingIterations.Equals(0)) break;
            }
        }
    }
}