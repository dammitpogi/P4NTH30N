...
public static class OrionStars {
	...
	private sealed record OrionStarsNetConfig(
		string BsIp,
		int WsPort,
		string WsProtocol,
		string GameProtocol,
		...
		try {
			...
			while (Iterations_LoginNotifications++ < 30) {
				...
				bool dialogBoxPopped = Screen
					...
					.Equals(Color.FromArgb(255, 0, 55, 135));
			...
			if (
				Screen
					.WaitForColor(new Point(715, 128), Color.FromArgb(255, 254, 242, 181), 15)
					.Equals(false)
			) {
	...
	private static OrionStarsNetConfig FetchNetConfig() {
		const string configUrl =
			"http://web.orionstars.org/hot_play/plat/config/hall/orionstars/config.json";
