        ...
        private void CleanCredentialDocument(BsonDocument credential, IMongoCollection<BsonDocument> collection, DataCleaningResult result, bool aggressive = false)
        {
            ...
            if (credential.Contains("Jackpots"))
            {
                ...
                if (jackpots.Contains("Grand"))
                {
                    ...
                    {
                        double newValue = !isValid ? 5000.0 : grandValue; // Safe default for aggressive mode
                        updates.Add(Builders<BsonDocument>.Update.Set("Jackpots.Grand", newValue));
                        result.Repairs.Add($"Repaired {username} Grand: {grandValue} -> {newValue}");
                    }
                }
                ...
                if (jackpots.Contains("Major"))
                {
                    ...
                    if (aggressive || !isValid)
                    {
                        double newValue = !isValid ? 500.0 : majorValue; // Safe default
                        updates.Add(Builders<BsonDocument>.Update.Set("Jackpots.Major", newValue));
                        result.Repairs.Add($"Repaired {username} Major: {majorValue} -> {newValue}");
                    }
                }
                ...
                if (jackpots.Contains("Minor"))
                {
                    ...
                    if (aggressive || !isValid)
                    {
                        double newValue = !isValid ? 100.0 : minorValue; // Safe default
                        updates.Add(Builders<BsonDocument>.Update.Set("Jackpots.Minor", newValue));
                        result.Repairs.Add($"Repaired {username} Minor: {minorValue} -> {newValue}");
                    }
                }
                ...
                if (jackpots.Contains("Mini"))
                {
                    ...
                    if (aggressive || !isValid)
                    {
                        double newValue = !isValid ? 25.0 : miniValue; // Safe default
                        updates.Add(Builders<BsonDocument>.Update.Set("Jackpots.Mini", newValue));
                        result.Repairs.Add($"Repaired {username} Mini: {miniValue} -> {newValue}");
                    }
                }
            }
            ...
            if (credential.Contains("Balance"))
            {
                ...
                if (aggressive || !isValid)
                {
                    double newBalance = !isValid ? 1000.0 : balance; // Safe default
                    updates.Add(Builders<BsonDocument>.Update.Set("Balance", newBalance));
                    result.Repairs.Add($"Repaired {username} Balance: {balance} -> {newBalance}");
                }
            }
            ...
            if (updates.Any())
            {
                var updateFilter = Builders<BsonDocument>.Filter.Eq("_id", credential["_id"]);
