...
public static class Gold777Flow {
    public static bool LoadSucessfully(ChromeDriver driver, CredentialRecord credential, SignalRecord signal) {
            ...
            Mouse.Move(credential.Settings.Gold777.Button_X, credential.Settings.Gold777.Button_Y);

            credential.Lock();
            signal.Acknowledge();
            while (slotsLoaded == false) {
                Mouse.Click(credential.Settings.Gold777.Button_X, credential.Settings.Gold777.Button_Y);
                ...
                if (buttonVerified == false) {
                    ...
                    if (System.Windows.Forms.Screen.PrimaryScreen != null) {
                        ...
                        foreach (int pageModifier in new int[] { 0, -1, 1, -2, 2 }) {
                            signal.Acknowledge();
                            int workingPage = credential.Settings.Gold777.Page + pageModifier;
