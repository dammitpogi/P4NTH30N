			...
			try
			{
				...
				if (credential.Jackpots != null)
				{
					...
					if (!grandValid || !majorValid || !minorValid || !miniValid)
					{
						hasCriticalIssues = true;
						if (!grandValid)
							issues.Add($"Grand: {credential.Jackpots.Grand}");
						if (!majorValid)
							issues.Add($"Major: {credential.Jackpots.Major}");
						if (!minorValid)
							issues.Add($"Minor: {credential.Jackpots.Minor}");
						if (!miniValid)
							issues.Add($"Mini: {credential.Jackpots.Mini}");
					}
				}
