...
/// <summary>
/// Validates P4NTHE0N configuration at startup and fails fast with clear error messages
...
{
	/// <summary>
	/// Validates the entire P4NTHE0N configuration tree.
	...
	public static List<string> Validate(P4NTHE0NOptions options)
	{
		List<string> errors = new();

		if (options is null)
		{
			errors.Add("P4NTHE0N configuration section is missing entirely.");
			return errors;
		}
	...
	public static void ValidateAndThrow(P4NTHE0NOptions options)
	{
		...
		if (errors.Count > 0)
		{
			...
			message.AppendLine($"P4NTHE0N configuration validation failed with {errors.Count} error(s):");
			message.AppendLine();
	...
	private static void ValidateDatabase(DatabaseOptions db, List<string> errors)
	{
		if (db is null)
		{
			errors.Add("P4NTHE0N:Database section is missing.");
			return;
		}

		if (string.IsNullOrWhiteSpace(db.ConnectionString))
			errors.Add("P4NTHE0N:Database:ConnectionString is required.");

		if (string.IsNullOrWhiteSpace(db.DatabaseName))
			errors.Add("P4NTHE0N:Database:DatabaseName is required.");

		if (db.ConnectionTimeoutSeconds <= 0)
			errors.Add("P4NTHE0N:Database:ConnectionTimeoutSeconds must be positive.");

		if (db.MaxPoolSize <= 0 || db.MaxPoolSize > 1000)
			errors.Add("P4NTHE0N:Database:MaxPoolSize must be between 1 and 1000.");
		...
		{
			errors.Add("P4NTHE0N:Database:ConnectionString must start with 'mongodb://' or 'mongodb+srv://'.");
		}
	}
	...
	private static void ValidateH0UND(H0UNDOptions h0und, List<string> errors)
	{
		...
		if (h0und.Polling.IntervalSeconds < 1)
			errors.Add("P4NTHE0N:H0UND:Polling:IntervalSeconds must be at least 1.");

		if (h0und.Polling.MaxConcurrent < 1)
			errors.Add("P4NTHE0N:H0UND:Polling:MaxConcurrent must be at least 1.");

		if (h0und.Polling.TimeoutSeconds < 1)
			errors.Add("P4NTHE0N:H0UND:Polling:TimeoutSeconds must be at least 1.");

		if (h0und.Analytics.ForecastWindowHours < 1)
			errors.Add("P4NTHE0N:H0UND:Analytics:ForecastWindowHours must be at least 1.");

		if (h0und.Analytics.MinDataPoints < 1)
			errors.Add("P4NTHE0N:H0UND:Analytics:MinDataPoints must be at least 1.");
	}
	...
	private static void ValidateH4ND(H4NDOptions h4nd, List<string> errors)
	{
		...
		if (h4nd.Selenium.ImplicitWaitSeconds < 0)
			errors.Add("P4NTHE0N:H4ND:Selenium:ImplicitWaitSeconds cannot be negative.");

		if (h4nd.Selenium.PageLoadTimeoutSeconds < 1)
			errors.Add("P4NTHE0N:H4ND:Selenium:PageLoadTimeoutSeconds must be at least 1.");

		if (h4nd.Selenium.ScriptTimeoutSeconds < 1)
			errors.Add("P4NTHE0N:H4ND:Selenium:ScriptTimeoutSeconds must be at least 1.");

		if (h4nd.RetryPolicy.MaxAttempts < 1)
			errors.Add("P4NTHE0N:H4ND:RetryPolicy:MaxAttempts must be at least 1.");

		if (h4nd.RetryPolicy.BackoffSeconds < 0)
			errors.Add("P4NTHE0N:H4ND:RetryPolicy:BackoffSeconds cannot be negative.");
	}
	...
	private static void ValidateSecurity(SecurityOptions security, List<string> errors)
	{
		...
		if (string.IsNullOrWhiteSpace(security.MasterKeyPath))
			errors.Add("P4NTHE0N:Security:MasterKeyPath is required when security section is configured.");

		if (security.Pbkdf2Iterations < 100_000)
			errors.Add("P4NTHE0N:Security:Pbkdf2Iterations must be at least 100,000 (OWASP minimum).");
	}
	...
	private static void ValidateRAG(RagOptions rag, List<string> errors)
	{
		...
		if (rag.EmbeddingDimensions <= 0)
			errors.Add("P4NTHE0N:RAG:EmbeddingDimensions must be positive.");
	}
	...
	private static void ValidateSafety(SafetyOptions safety, List<string> errors)
	{
		...
		if (safety.DailyLossLimit <= 0)
			errors.Add("P4NTHE0N:Safety:DailyLossLimit must be positive.");

		if (safety.MaxConsecutiveLosses < 1)
			errors.Add("P4NTHE0N:Safety:MaxConsecutiveLosses must be at least 1.");

		if (safety.BalanceAlertThreshold <= 0 || safety.BalanceAlertThreshold > 1.0m)
			errors.Add("P4NTHE0N:Safety:BalanceAlertThreshold must be between 0 and 1.0 (percentage).");
	}
}
