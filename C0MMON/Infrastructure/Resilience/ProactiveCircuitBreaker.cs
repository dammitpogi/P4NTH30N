...
namespace P4NTH30N.C0MMON.Infrastructure.Resilience
{
    public class ProactiveCircuitBreaker
    {
        ...
        {
            if (triageInfo.IsTriaged) return false;
            ...
            return failuresInWindow.Count() >= _settings.FailureThreshold || 
                   CalculateFailureVelocity(failuresInWindow) > _settings.FailureRateThreshold;
        }

        private double CalculateFailureVelocity(System.Collections.Generic.IEnumerable<DateTime> failures)
        {
            if (failures == null || !failures.Any()) return 0.0;
            ...
            if (timeSpan.TotalSeconds < 1) return orderedFailures.Count();
