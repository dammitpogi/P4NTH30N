        ...
        while (true) {
            try {
                ...
                List<Game> games = Game.GetAll()
                    .FindAll(x => credentials.Any(y => x.House.Equals(y.House) && x.Name.Equals(y.Game)));
                ...
                games.ForEach(delegate (Game game) {
                    if (game.Enabled == true) {
                        ...
                        if (game.Jackpots.Grand != previousGrand) {
                            if (game.Jackpots.Grand > previousGrand) {
                                ...
                                if (game.DPD.Data.Count > 2) {
                                    float minutes = Convert.ToSingle(game.DPD.Data[game.DPD.Data.Count - 1].Timestamp.Subtract(game.DPD.Data[0].Timestamp).TotalMinutes);
                                    double dollars = game.DPD.Data[game.DPD.Data.Count - 1].Grand - game.DPD.Data[0].Grand;
                                    float days = minutes / Convert.ToSingle(TimeSpan.FromDays(1).TotalMinutes);
                                    double dollarsPerDay = dollars / days;

                                    if (dollarsPerDay > 5 && game.DPD.History.Count.Equals(0) == false)
                                        game.DPD.Average = game.DPD.History.Average(x => x.Average);
                                    else game.DPD.Average = dollarsPerDay;
                                }
                            } else {
                                if (game.DPD.History.Count.Equals(0).Equals(false) && game.DPD.History[^1].Data[^1].Grand <= previousGrand) {
                                    game.DPD.Data = []; game.DPD.Data.Add(new DPD_Data(game.Jackpots.Grand));
                                    game.DPD.Average = 0F;
                                } else {
                        ...
                        if (game.DPD.Average.Equals(0) && game.DPD.History.Count > 0 && game.DPD.History[^1].Average > 0.1) {
                        ...
                        if (game.DPD.Average > 0.1) {
                            ...
                            double MinutesToGrand = Math.Max((game.Thresholds.Grand - (game.Jackpots.Grand + estimatedGrowth)) / DPM, 0);
                            double MinutesToMajor = Math.Max((game.Thresholds.Major - (game.Jackpots.Major + estimatedGrowth)) / DPM, 0);
                            double MinutesToMinor = Math.Max((game.Thresholds.Minor - (game.Jackpots.Minor + estimatedGrowth)) / DPM, 0);
                            double MinutesToMini = Math.Max((game.Thresholds.Mini - (game.Jackpots.Mini + estimatedGrowth)) / DPM, 0);

                            if (game.Settings.SpinGrand)
                                new Jackpot(game, "Grand", game.Jackpots.Grand, game.Thresholds.Grand, 4, DateTime.UtcNow.AddMinutes(MinutesToGrand)).Save();

                            if (game.Settings.SpinMajor)
                                new Jackpot(game, "Major", game.Jackpots.Major, game.Thresholds.Major, 3, DateTime.UtcNow.AddMinutes(MinutesToMajor)).Save();

                            if (game.Settings.SpinMinor)
                                new Jackpot(game, "Minor", game.Jackpots.Minor, game.Thresholds.Minor, 2, DateTime.UtcNow.AddMinutes(MinutesToMinor)).Save();
                                
                            if (game.Settings.SpinMini) 
                                new Jackpot(game, "Mini", game.Jackpots.Mini, game.Thresholds.Mini, 1, DateTime.UtcNow.AddMinutes(MinutesToMini)).Save();
                        }
                    }
                });

                List<Signal> signals = Signal.GetAll();
                List<Jackpot> jackpots = Jackpot.GetAll()
                    .FindAll(x => x.EstimatedDate < DateLimit)
                    .FindAll(x => games.Any(y => y.Enabled && x.Game.Equals(y.Name) && x.House.Equals(y.House)));
                ...
                foreach (Jackpot jackpot in jackpots) {
                    ...
                    if (game.Settings.SpinGrand.Equals(false) && jackpot.Category.Equals("Grand")) continue;
                    if (game.Settings.SpinMajor.Equals(false) && jackpot.Category.Equals("Major")) continue;
                    if (game.Settings.SpinMinor.Equals(false) && jackpot.Category.Equals("Minor")) continue;
                    if (game.Settings.SpinMini.Equals(false) && jackpot.Category.Equals("Mini")) continue;

                    // Console.WriteLine(game.House);
                    if (game.DPD.Average > 0.01) {
                        predictions.Add(jackpot);
                        double capacity = jackpot.Threshold - jackpot.Category switch { "Grand" => 1500, "Major" => 500, "Minor" => 100, "Mini" => 20, _ => 0 };
                        double daysToAdd = capacity / game.DPD.Average; int iterations = 1;
                        for (DateTime i = jackpot.EstimatedDate.AddDays(daysToAdd); i < DateLimit; i = i.AddDays(daysToAdd)) {
                            predictions.Add(new Jackpot(game, jackpot.Category, jackpot.Current, jackpot.Threshold + (capacity * iterations++), jackpot.Priority, i));
                        }

                        jackpot.Current += DateTime.UtcNow.Subtract(jackpot.LastUpdated).TotalMinutes * jackpot.DPM;
                        jackpot.EstimatedDate = DateTime.UtcNow.AddMinutes(Math.Max((jackpot.Threshold - jackpot.Current) / jackpot.DPM, 0));
                ...
                predictions.ForEach(delegate (Jackpot jackpot) {
                    ...
                    else potentials.Add(jackpot.House, jackpot.Threshold);
                });
                ...
                credentials.ForEach(delegate (Credential credential) {
                    if (recommendations.ContainsKey(credential.House) == false) recommendations.Add(credential.House, 0);
                });
