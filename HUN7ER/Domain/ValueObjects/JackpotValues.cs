...
public class JackpotValues
{
	...
	public bool IsValid =>
		!double.IsNaN(Grand)
		&& !double.IsInfinity(Grand)
		&& Grand >= 0
		&& !double.IsNaN(Major)
		&& !double.IsInfinity(Major)
		&& Major >= 0
		&& !double.IsNaN(Minor)
		&& !double.IsInfinity(Minor)
		&& Minor >= 0
		&& !double.IsNaN(Mini)
		&& !double.IsInfinity(Mini)
		&& Mini >= 0;
	...
	public void ClampToValid()
	{
		...
		if (double.IsNaN(Major) || double.IsInfinity(Major) || Major < 0)
			Major = 0;
		if (double.IsNaN(Minor) || double.IsInfinity(Minor) || Minor < 0)
			Minor = 0;
		if (double.IsNaN(Mini) || double.IsInfinity(Mini) || Mini < 0)
			Mini = 0;
	}
