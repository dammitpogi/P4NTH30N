...
describe('agent alias backward compatibility', () => {
  test("applies 'explore' config to 'explorer' agent", () => {
    const config: PluginConfig = {
      agents: {
        explore: { currentModel: 'test/old-explore-model' },
      },
    };
  ...
  test("applies 'frontend-ui-ux-engineer' config to 'designer' agent", () => {
    const config: PluginConfig = {
      agents: {
        'frontend-ui-ux-engineer': { currentModel: 'test/old-frontend-model' },
      },
    };
    const agents = createAgents(config);
  ...
  test('new name takes priority over old alias', () => {
    const config: PluginConfig = {
      agents: {
        explore: { currentModel: 'old-model' },
        explorer: { currentModel: 'new-model' },
      },
    };
    const agents = createAgents(config);
    const explorer = agents.find((a) => a.name === 'explorer');
  ...
  test('new agent names work directly', () => {
    const config: PluginConfig = {
      agents: {
        explorer: { currentModel: 'direct-explorer' },
        designer: { currentModel: 'direct-designer' },
      },
    };
    const agents = createAgents(config);
...
describe('fixer agent model selection', () => {
  test('fixer uses default model when no config provided', () => {
    ...
    expect(fixer?.config.model).toBe('openrouter-free/openrouter/free');
  });

  test('fixer uses its own model when explicitly configured', () => {
    const config: PluginConfig = {
      agents: {
        librarian: { currentModel: 'librarian-model' },
        fixer: { currentModel: 'fixer-specific-model' },
      },
    };
    const agents = createAgents(config);
    const fixer = agents.find((a) => a.name === 'fixer');
...
describe('orchestrator agent', () => {
  ...
  test('orchestrator does not enforce hardcoded question permission', () => {
    const agents = createAgents();
    const orchestrator = agents.find((a) => a.name === 'orchestrator');
    expect((orchestrator?.config.permission as any)?.question).toBeUndefined();
  });
  ...
  test('orchestrator accepts overrides', () => {
    const config: PluginConfig = {
      agents: {
        orchestrator: {
          currentModel: 'custom-orchestrator-model',
          temperature: 0.3,
        },
      },
    };
