...
const PACKAGE_NAME = 'oh-my-opencode-theseus';
...
export function stripJsonComments(json: string): string {
  ...
  // Remove trailing commas while respecting string literals.
  let cleaned = '';
  i = 0;
  inString = false;
  escaped = false;
  ...
  while (i < withoutComments.length) {
    ...
    if (ch === ',') {
      ...
      const nextToken = withoutComments[j];
...
export async function addPluginToOpenCodeConfig(): Promise<ConfigMergeResult> {
  ...
  try {
    ...
    // Remove existing oh-my-opencode-theseus entries
...
export function detectCurrentConfig(): DetectedConfig {
  ...
  if (liteConfig && typeof liteConfig === 'object') {
    ...
    const agents = presets?.[presetName] as
      | Record<string, { currentModel?: string }>
      | undefined;

    if (agents) {
      const models = Object.values(agents)
        .map((a) => a?.currentModel)
        .filter(Boolean);
