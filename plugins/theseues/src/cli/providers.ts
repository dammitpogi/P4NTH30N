...
export const MODEL_MAPPINGS = {
  kimi: {
    orchestrator: { model: 'openrouter/moonshotai/kimi-k2-thinking' },
    oracle: { model: 'openrouter/moonshotai/kimi-k2', variant: 'high' },
    librarian: { model: 'openrouter/moonshotai/kimi-k2-thinking', variant: 'low' },
    explorer: { model: 'openrouter/moonshotai/kimi-k2', variant: 'low' },
    designer: { model: 'zai-coding-plan/glm-4.7', variant: 'medium' },
    fixer: { model: 'openrouter/moonshotai/kimi-k2.5', variant: 'low' },
  },
  openai: {
...
export function generateLiteConfig(
  ...
  const createAgentConfig = (
    agentName: string,
    modelInfo: { model: string; variant?: string; models?: string[] },
  ) => {
    const isOrchestrator = agentName === 'orchestrator';
    ...
    return {
      currentModel: modelInfo.model,
      variant: modelInfo.variant,
      models: modelInfo.models, // fallback chain
      skills,
      mcps:
        DEFAULT_AGENT_MCPS[agentName as keyof typeof DEFAULT_AGENT_MCPS] ?? [],
    };
      ...
      Object.entries(installConfig.dynamicModelPlan.agents).map(
        ([agentName, assignment]) => [
          ...
          createAgentConfig(
            ...
            {
              ...assignment,
              models: installConfig.dynamicModelPlan?.chains[agentName],
            } as { model: string; variant?: string; models?: string[] },
          ),
        ],
      ),
    );
  ...
  const attachFallbackConfig = (presetAgents: Record<string, unknown>) => {
    ...
    for (const agentName of AGENT_NAMES) {
      const currentModel = (
        presetAgents[agentName] as { currentModel?: string } | undefined
      )?.currentModel;
      ...
      if (chain.length > 0) {
        ...
        const existing = presetAgents[agentName] as
          | { models?: string[] }
          | undefined;
        if (existing && typeof existing === 'object') {
          existing.models = chain;
        }
      }
    }
