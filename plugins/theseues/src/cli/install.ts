...
export async function install(args: InstallArgs): Promise<number> {
  ...
  if (!args.tui) {
    ...
    if (errors.length > 0) {
      ...
      console.log();
      printInfo(
        'Usage: bunx oh-my-opencode-theseus install --no-tui --kimi=<yes|no> --openai=<yes|no> --anthropic=<yes|no> --copilot=<yes|no> --zai-plan=<yes|no> --antigravity=<yes|no> --chutes=<yes|no> --tmux=<yes|no>',
      );
      console.log();
      return 1;
    }
