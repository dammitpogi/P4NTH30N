...
The `src/tools/` directory provides the core tool implementations for the oh-my-opencode-theseus plugin. It exposes three main categories of tools:
...
- `background_cancel` passes an explicit cancel reason to the manager:
  - `Cancelled via background_cancel (session <parentSessionId>)` when `toolContext.sessionID` is available
  - otherwise `Cancelled via background_cancel`
- The manager stores this reason on the cancelled task and includes it in the parent-session completion notification.
```
...
- **Install location**: `~/.cache/oh-my-opencode-theseus/bin/rg` (Linux/macOS), `%LOCALAPPDATA%\oh-my-opencode-theseus\bin\rg.exe` (Windows)
...
- **Install location**: `~/.cache/oh-my-opencode-theseus/bin/sg` (Linux/macOS), `%LOCALAPPDATA%\oh-my-opencode-theseus\bin\sg.exe` (Windows)
