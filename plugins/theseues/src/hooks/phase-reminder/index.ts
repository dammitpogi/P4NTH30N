 ...
 */
const PHASE_REMINDER = `<reminder>⚠️ WORKFLOW: 0:INTEL→1:SETUP→2:INVESTIGATE→3:PLAN→[USER APPROVE]→4:BUILD→5:VERIFY→6:CONFIRM→7:DOCUMENT
⚡ ALWAYS use background_task (not regular Task) for delegation - enables fallbacks
Key: Librarian writes ⚡ Fixer programs ⚡ Explorer discovers ⚡ Oracle advises ⚡ Designer plans
Delegation: background_task→continue working→check background_output after completion notification (avoid polling loops) | Never idle | Fixer fires ONCE
</reminder>`;
