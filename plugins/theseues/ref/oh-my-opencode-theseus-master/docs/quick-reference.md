...
Complete reference for oh-my-opencode-theseus configuration and capabilities.
...
Edit `~/.config/opencode/oh-my-opencode-theseus.json` (or `.jsonc`) and change the `preset` field:
...
**Installation:**
```bash
bunx oh-my-opencode-theseus install --antigravity=yes --opencode-free=yes --opencode-free-model=auto
```
...
You can customize which skills each agent is allowed to use in `~/.config/opencode/oh-my-opencode-theseus.json` (or `.jsonc`).
...
You can configure MCP access in your plugin configuration file: `~/.config/opencode/oh-my-opencode-theseus.json` (or `.jsonc`).
...
1. **Enable tmux integration** in `oh-my-opencode-theseus.json` (or `.jsonc`):
...
| `~/.config/opencode/oh-my-opencode-theseus.json` or `.jsonc` | Plugin settings (agents, tmux, MCPs) |
| `.opencode/oh-my-opencode-theseus.json` or `.jsonc` | Project-local plugin overrides (optional) |
...
You can customize agent prompts by creating markdown files in `~/.config/opencode/oh-my-opencode-theseus/`:
...
```
~/.config/opencode/oh-my-opencode-theseus/
...
**File Priority:**
1. `oh-my-opencode-theseus.jsonc` (preferred if exists)
2. `oh-my-opencode-theseus.json` (fallback)
...
### Plugin Config (`oh-my-opencode-theseus.json` or `oh-my-opencode-theseus.jsonc`)
