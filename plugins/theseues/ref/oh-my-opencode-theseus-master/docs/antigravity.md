...
1. Install with Antigravity support:
   ```bash
   bunx oh-my-opencode-theseus install --antigravity=yes
   ```
...
If you prefer to configure manually, edit `~/.config/opencode/oh-my-opencode-theseus.json` (or `.jsonc`) and add a pure Antigravity preset:
...
# Reinstall plugin
bunx oh-my-opencode-theseus install --antigravity=yes --no-tui --kimi=no --openai=no --tmux=no --skills=no
```
...
# Or edit ~/.config/opencode/oh-my-opencode-theseus.json (or .jsonc)
