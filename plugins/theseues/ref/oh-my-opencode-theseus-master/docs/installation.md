...
Complete installation instructions for oh-my-opencode-theseus.
...
```bash
bunx oh-my-opencode-theseus@latest install
```

Or use non-interactive mode:

```bash
bunx oh-my-opencode-theseus@latest install --no-tui --kimi=yes --openai=yes --antigravity=yes --chutes=yes --opencode-free=yes --opencode-free-model=auto --tmux=no --skills=yes
```
...
```bash
bunx oh-my-opencode-theseus install --kimi=yes --openai=yes --antigravity=yes --chutes=yes --opencode-free=yes --opencode-free-model=auto
```
...
https://raw.githubusercontent.com/alvinunreal/oh-my-opencode-theseus/refs/heads/master/README.md
```
...
If you're an LLM Agent helping set up oh-my-opencode-theseus, follow these steps.
...
```bash
bunx oh-my-opencode-theseus@latest install --no-tui --kimi=<yes|no> --openai=<yes|no> --antigravity=<yes|no> --chutes=<yes|no> --opencode-free=<yes|no> --opencode-free-model=<id|auto> --tmux=<yes|no> --skills=<yes|no>
```
...
# OpenAI only
bunx oh-my-opencode-theseus@latest install --no-tui --kimi=no --openai=yes --antigravity=no --chutes=no --opencode-free=no --tmux=no --skills=yes

# OpenCode free models only (auto-select)
bunx oh-my-opencode-theseus@latest install --no-tui --kimi=no --openai=no --antigravity=no --chutes=no --opencode-free=yes --opencode-free-model=auto --tmux=no --skills=yes

# OpenCode free models + OpenAI (manual primary model)
bunx oh-my-opencode-theseus@latest install --no-tui --kimi=no --openai=yes --antigravity=no --chutes=no --opencode-free=yes --opencode-free-model=opencode/gpt-5-nano --tmux=no --skills=yes
```

The installer automatically:
- Adds the plugin to `~/.config/opencode/opencode.json`
- Generates agent model mappings in `~/.config/opencode/oh-my-opencode-theseus.json` (or `.jsonc`)

**Crucial Advice for the User:**
- They can easily assign **different models to different agents** by editing `~/.config/opencode/oh-my-opencode-theseus.json` (or `.jsonc`).
- If they add a new provider later, they just need to update this file.
- Read generated `~/.config/opencode/oh-my-opencode-theseus.json` (or `.jsonc`) file and report the model configuration.
...
```bash
bunx oh-my-opencode-theseus@latest install --help
```

Then manually create the config files at:
- `~/.config/opencode/oh-my-opencode-theseus.json` (or `.jsonc`)
...
2. Verify your config file exists and is valid:
   ```bash
   cat ~/.config/opencode/oh-my-opencode-theseus.json
   ```
...
1. **Remove the plugin from your OpenCode config**:
   ...
   Edit `~/.config/opencode/opencode.json` and remove `"oh-my-opencode-theseus"` from the `plugin` array.

2. **Remove configuration files (optional)**:
   ```bash
   rm -f ~/.config/opencode/oh-my-opencode-theseus.json
   rm -f .opencode/oh-my-opencode-theseus.json
   ```
