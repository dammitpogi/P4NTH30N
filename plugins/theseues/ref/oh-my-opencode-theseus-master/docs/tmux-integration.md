...
Complete guide for using tmux integration with oh-my-opencode-theseus to watch agents work in real-time through automatic pane spawning.
...
Edit `~/.config/opencode/oh-my-opencode-theseus.json` (or `.jsonc`):
...
Configure tmux behavior in `~/.config/opencode/oh-my-opencode-theseus.json` (or `.jsonc`):
...
1. **Verify tmux integration is enabled:**
   ```bash
    cat ~/.config/opencode/oh-my-opencode-theseus.json | grep tmux # (or .jsonc)
   ```
