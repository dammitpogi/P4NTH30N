...
export function getCacheDir(): string {
  if (process.platform === 'win32') {
    ...
    return join(base, 'oh-my-opencode-theseus', 'bin');
  }
...
export async function downloadAstGrep(
  ...
  if (!platformInfo) {
    console.error(
      `[oh-my-opencode-theseus] Unsupported platform for ast-grep: ${platformKey}`,
    );
    return null;
  }
  ...
  console.log(`[oh-my-opencode-theseus] Downloading ast-grep binary...`);
  ...
  try {
    ...
    console.log(`[oh-my-opencode-theseus] ast-grep binary ready.`);

    return binaryPath;
  } catch (err) {
    console.error(
      `[oh-my-opencode-theseus] Failed to download ast-grep: ${err instanceof Error ? err.message : err}`,
    );
    return null;
  }
}
