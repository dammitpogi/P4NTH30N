...
function printHeader(isUpdate: boolean): void {
  console.log();
  console.log(
    `${BOLD}oh-my-opencode-theseus ${isUpdate ? 'Update' : 'Install'}${RESET}`,
  );
...
async function runInteractiveMode(
  ...
  try {
    ...
    console.log();
    console.log(`${BOLD}oh-my-opencode-theseus Setup${RESET}`);
...
async function runInstall(config: InstallConfig): Promise<number> {
  ...
  if (!modelsOnly) {
    printStep(step++, totalSteps, 'Adding oh-my-opencode-theseus plugin...');
    if (resolvedConfig.dryRun) {
  ...
  printStep(step++, totalSteps, 'Writing oh-my-opencode-theseus configuration...');
  if (resolvedConfig.dryRun) {
...
export async function install(args: InstallArgs): Promise<number> {
  ...
  if (!args.tui) {
    ...
    if (errors.length > 0) {
      ...
      console.log();
      printInfo(
        'Usage: bunx oh-my-opencode-theseus install --no-tui --kimi=<yes|no> --openai=<yes|no> --anthropic=<yes|no> --copilot=<yes|no> --zai-plan=<yes|no> --antigravity=<yes|no> --chutes=<yes|no> --balanced-spend=<yes|no> --tmux=<yes|no>',
      );
      console.log();
      return 1;
    }
