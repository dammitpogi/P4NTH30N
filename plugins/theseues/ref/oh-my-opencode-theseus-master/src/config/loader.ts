...
const PROMPTS_DIR_NAME = 'oh-my-opencode-theseus';
...
function loadConfigFromPath(configPath: string): PluginConfig | null {
  try {
    ...
    if (
      ...
      console.warn(
        `[oh-my-opencode-theseus] Error reading config from ${configPath}:`,
        error.message,
      );
    }
    return null;
  ...
  if (config.preset) {
    ...
    if (preset) {
      ...
      const availablePresets = config.presets
        ...
        : 'none';
      console.warn(
        `[oh-my-opencode-theseus] Preset "${config.preset}" not found (from ${presetSource}). Available presets: ${availablePresets}`,
      );
    }
  }
...
export function loadAgentPrompt(agentName: string): {
  ...
  if (fs.existsSync(promptPath)) {
    try {
      ...
      console.warn(
        `[oh-my-opencode-theseus] Error reading prompt file ${promptPath}:`,
  ...
  if (fs.existsSync(appendPromptPath)) {
    try {
      ...
      console.warn(
        `[oh-my-opencode-theseus] Error reading append prompt file ${appendPromptPath}:`,
        error instanceof Error ? error.message : String(error),
      );
    }
  }
