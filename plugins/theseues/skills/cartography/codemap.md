...
Defines the Cartography skill: a deterministic workflow for maintaining directory-scoped `codemap.md` files and a root repository atlas (`codemap.md`) using the file/folder hash state stored in `.theseues/cartography.json`.
...
- State-based change detection: `skills/cartography/scripts/cartographer.py` stores hashes for selected files/folders in `.theseues/cartography.json` so updates can be targeted to affected scopes.
- Directory-scoped maps: each mapped directory owns a `codemap.md` describing only artifacts in that subtree.

## Flow

1. Initialize state (once): `cartographer.py init --root ./ ...` selects files (include/exclude/exception patterns), writes `.theseues/cartography.json`, and creates empty `codemap.md` templates.
...
- Uses `skills/cartography/scripts/cartographer.py` as the sole state source of truth (`.theseues/cartography.json`) for detecting affected scopes.
