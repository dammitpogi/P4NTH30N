...
public sealed class ParallelDelegation {
	...
	/// </summary>
	public async Task<DelegationBatchResult> DelegateAsync(
		IReadOnlyList<ResearchBrief> briefs,
		...
		List<Task> tasks = briefs.Select(async brief => {
		...
		batch.Status = batch.FailedCount == 0 && batch.TimedOutCount == 0
	...
	/// </summary>
	public static ResearchBrief CreateBrief(
		string agentName,
	...
	public static AggregatedResult AggregateResults(DelegationBatchResult batch) {
		return new AggregatedResult {
			...
			Outputs = batch.Results
				.Where(r => r.Status == DelegationStatus.Succeeded && r.Output != null)
				.ToDictionary(r => r.Brief.AgentName, r => r.Output!),
			Errors = batch.Results
				.Where(r => r.Status != DelegationStatus.Succeeded)
				.Select(r => $"[{r.Brief.AgentName}] {r.Error ?? r.Status.ToString()}")
				.ToList(),
		};
	}

	private async Task<DelegationResult> ExecuteBriefAsync(
		ResearchBrief brief,
		...
		try {
			...
			using CancellationTokenSource linked = CancellationTokenSource.CreateLinkedTokenSource(
				cancellationToken, briefCts.Token);
...
public sealed class DelegationResult {
	...
	public static DelegationResult Skipped(ResearchBrief brief, string reason) => new() {
		Brief = brief,
		Status = DelegationStatus.Skipped,
		Error = reason,
	};

	public static DelegationResult TimedOut(ResearchBrief brief) => new() {
		Brief = brief,
...
public enum DelegationStatus { Succeeded, Failed, TimedOut, Skipped }
public enum BatchStatus { AllSucceeded, PartialSuccess, AllFailed, CircuitBreakerOpen }
