...
public sealed class AgentIntegration {
	...
	public AgentDelegation DelegateTask(string taskDescription, string requiredCapability) {
		AgentProfile? agent = _agents.Values.FirstOrDefault(a =>
			a.Capabilities.Contains(requiredCapability));

		if (agent == null) {
			return new AgentDelegation {
				Status = DelegationStatus.NoCapableAgent,
				Error = $"No agent has capability '{requiredCapability}'",
			};
		}
	...
	/// </summary>
	public List<AgentDelegation> DelegateBatch(
		IEnumerable<(string description, string capability)> tasks) {
...
public enum DelegationStatus { Delegated, NoCapableAgent, AgentBusy, Failed }
