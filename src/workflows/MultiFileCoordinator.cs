...
public sealed class MultiFileCoordinator
{
	...
	public void QueueEdit(FileEdit edit)
	{
		...
		{
			throw new InvalidOperationException($"Maximum edits per turn ({_config.MaxEditsPerTurn}) exceeded. " + "Flush current batch before adding more.");
		}
	...
	{
		CoordinationResult result = new() { BatchId = Guid.NewGuid().ToString("N")[..8], TotalEdits = _pendingEdits.Count };
		...
		foreach (FileEdit edit in _pendingEdits)
		{
			...
			SemaphoreSlim fileLock = _fileLocks.GetOrAdd(NormalizePath(edit.FilePath), _ => new SemaphoreSlim(1, 1));
		...
		result.Status = result.FailedCount == 0 && consistencyErrors.Count == 0 ? CoordinationStatus.Succeeded : CoordinationStatus.PartialFailure;
		...
		foreach (IGrouping<string, FileEdit> group in groupedByFile)
		{
			...
			for (int i = 0; i < fileEdits.Count; i++)
			{
				for (int j = i + 1; j < fileEdits.Count; j++)
				{
					...
					{
						errors.Add($"Conflicting edits on {fileEdits[i].FilePath}: " + $"edit '{fileEdits[i].Description}' overlaps with '{fileEdits[j].Description}'");
					}
				}
			}
		}
		...
		foreach (FileEdit edit in edits)
		{
			foreach (string refPath in edit.ReferencedFiles)
			{
				FileEdit? conflicting = edits.FirstOrDefault(e =>
					NormalizePath(e.FilePath) == NormalizePath(refPath) && e.ReferencedFiles.Any(r => NormalizePath(r) == NormalizePath(edit.FilePath))
				);

				if (conflicting != null)
				{
					errors.Add($"Circular reference between {edit.FilePath} and {conflicting.FilePath}");
				}
			}
		}
	...
	public static List<string> CheckConsistency(List<FileEditResult> results)
	{
		...
		foreach (FileEditResult result in succeeded)
		{
			...
			{
				bool depExists = succeeded.Any(r => NormalizePath(r.Edit.FilePath) == NormalizePath(dep));
	...
	private static async Task<FileEditResult> ExecuteEditAsync(FileEdit edit, CancellationToken cancellationToken)
	{
		try
		{
			switch (edit.Type)
			...
			return new FileEditResult { Edit = edit, Succeeded = true };
		}
		catch (Exception ex)
		{
			return new FileEditResult
