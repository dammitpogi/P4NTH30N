...
public sealed class CodeGenerationValidator {
			...
			if (inMethod) {
				...
				if (braceDepth <= 0 && methodStartLine > 0) {
					...
					if (methodLines > _config.MaxMethodLines) {
						result.AddWarning(
							$"Method '{currentMethod}' at line {methodStartLine} is {methodLines} lines " +
							$"(max {_config.MaxMethodLines})");
					}
					inMethod = false;
				}
			}
	...
	public static void ValidateNaming(string sourceCode, CodeValidationResult result) {
		...
		MatchCollection privateFields = Regex.Matches(sourceCode,
			@"private\s+(?:readonly\s+)?(?:\w+)\s+(\w+)\s*[;=]");
		...
		MatchCollection publicMethods = Regex.Matches(sourceCode,
	...
	public void AnalyzeComplexity(string sourceCode, CodeValidationResult result) {
		...
		if (result.EstimatedCyclomaticComplexity > _config.MaxCyclomaticComplexity) {
			result.AddWarning(
				$"Estimated cyclomatic complexity {result.EstimatedCyclomaticComplexity} " +
				$"exceeds limit ({_config.MaxCyclomaticComplexity})");
		}
	}
	...
	private static bool IsMethodSignature(string line) {
		return Regex.IsMatch(line,
			@"(public|private|protected|internal)\s+(static\s+)?(async\s+)?(override\s+)?\w+.*\(.*\)\s*\{?$");
	}
