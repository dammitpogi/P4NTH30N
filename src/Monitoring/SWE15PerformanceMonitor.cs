...
namespace P4NTHE0N.SWE.Monitoring;
...
public sealed class SWE15PerformanceMonitor
{
	...
	public void RecordResponseTime(string operation, long elapsedMs)
	{
		...
		{
			RaiseAlert(AlertSeverity.Warning, $"Response time {elapsedMs}ms exceeds {threshold}ms threshold for {operation}");
		}
	...
	public ContextStatus UpdateContextSize(long estimatedTokens)
	{
		...
		double usage = _config.MaxContextTokens > 0 ? (double)estimatedTokens / _config.MaxContextTokens : 0.0;

		if (usage >= 0.95)
		{
			RaiseAlert(AlertSeverity.Critical, $"Context at {usage:P0} capacity ({estimatedTokens} tokens). Session reset required.");
			return ContextStatus.MustReset;
		}

		if (usage >= 0.90)
		{
			RaiseAlert(AlertSeverity.Warning, $"Context at {usage:P0} capacity ({estimatedTokens} tokens). Compress recommended.");
	...
	public TurnStatus AdvanceTurn()
	{
		...
		{
			RaiseAlert(AlertSeverity.Critical, $"Turn {_currentTurn} exceeds hard limit ({_config.HardResetTurns}). Session reset required.");
			return TurnStatus.MustReset;
		}

		if (_currentTurn >= _config.SoftResetTurns)
		{
			RaiseAlert(AlertSeverity.Warning, $"Turn {_currentTurn} approaching limit ({_config.HardResetTurns}). Plan for reset.");
	...
	public void RecordError(string operation, string errorMessage)
	{
		...
		int recentErrors = _metrics.Where(m => m.IsError && m.TurnNumber >= _currentTurn - 5).Count();

		if (recentErrors >= 3)
		{
			IsDegraded = true;
			RaiseAlert(AlertSeverity.Critical, $"Error pattern detected: {recentErrors} errors in last 5 turns");
		}
	}
	...
	public PerformanceSummary GetSummary()
	{
		...
		return new PerformanceSummary
		{
			...
			ContextUsagePercent = _config.MaxContextTokens > 0 ? (double)_estimatedContextTokens / _config.MaxContextTokens * 100 : 0.0,
			TotalMetrics = allMetrics.Count,
			ErrorCount = allMetrics.Count(m => m.IsError),
			AverageResponseMs = responseMetrics.Count > 0 ? responseMetrics.Average(m => m.DurationMs) : 0.0,
			P95ResponseMs = responseMetrics.Count > 0 ? Percentile(responseMetrics.Select(m => m.DurationMs).ToList(), 95) : 0.0,
			MaxResponseMs = responseMetrics.Count > 0 ? responseMetrics.Max(m => m.DurationMs) : 0,
	...
	{
		if (_estimatedContextTokens <= 50_000)
			return 0.0;
