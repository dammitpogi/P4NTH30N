# Tool Output: tool_c6dd162e9001P7PWwnNHH0SeyZ
**Date**: 2026-02-17 22:56:04 UTC
**Size**: 86,376 bytes
        ...
        "triggers": [
          {
            ...
            "apiLatency": "<500ms",
            "workerUtil": "<60%",
            "behavior": "Full signal generation"
          },
          {
            "level": "Reduced",
            "apiLatency": ">500ms",
            "workerUtil": ">60%",
            "behavior": "Batch signals, 70% capacity"
          },
          {
            "level": "Minimal",
            "apiLatency": ">1000ms",
            "workerUtil": ">80%",
            "behavior": "Priority 3-5 only, 50% capacity"
          },
          {
            "level": "Emergency",
            "apiLatency": ">2000ms",
            "workerUtil": ">90%",
            "behavior": "Halt automation"
          }
        ],
        "behaviors": {
          "ApplyLevelBehavior": "Adjust worker pool size, signal queue priority, mongo write concern",
          "NotifySubscribers": "All H4ND instances receive degradation events via SignalR or polling"
        },
        "codeExample": "\npublic class SystemDegradationManager : IDegradationManager {\n  public event Action<DegradationLevel, DegradationLevel>? OnDegradationChanged;\n  \n  public async Task<DegradationLevel> CheckDegradation() {\n    var metrics = await _metrics.Collect();\n    var newLevel = CalculateLevel(metrics);\n    \n    if (newLevel != _currentLevel) {\n      if (_cooldownExpired) {\n        OnDegradationChanged?.Invoke(_currentLevel, newLevel);\n        _currentLevel = newLevel;\n        ApplyLevelBehavior(newLevel);\n      }\n    }\n    return _currentLevel;\n  }\n  \n  private DegradationLevel CalculateLevel(SystemMetrics m) {\n    if (m.ApiLatency > 2000 || m.WorkerUtil > 90) return DegradationLevel.Emergency;\n    if (m.ApiLatency > 1000 || m.WorkerUtil > 80) return DegradationLevel.Minimal;\n    if (m.ApiLatency > 500  || m.WorkerUtil > 60) return DegradationLevel.Reduced;\n    return DegradationLevel.Normal;\n  }\n}"
      }
    },
    {
      "_id": {
