...
public sealed record Threshold
{
	...
	public Threshold(Money grand, Money major, Money minor, Money mini)
	{
		Grand = grand;
		Major = major;
		Minor = minor;
	...
	public Money GetByTier(JackpotTier tier) => tier switch
	{
		...
		_ => throw new DomainException($"Unsupported jackpot tier '{tier}'.", "Threshold.GetByTier"),
	};

	public Threshold WithTier(JackpotTier tier, Money value) => tier switch
	{
		...
		_ => throw new DomainException($"Unsupported jackpot tier '{tier}'.", "Threshold.WithTier"),
	};
}
